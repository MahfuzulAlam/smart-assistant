...
class OpenAIClient {
	...
	public function send_chat_message( $messages ) {
		...
		if ( empty( $api_key ) ) {
			return new \WP_Error( 'no_api_key', __( 'OpenAI API key is not configured.', 'smart-assistant' ) );
		}
		...
		if ( is_wp_error( $response ) ) {
			error_log( 'Smart Assistant API Error: ' . $response->get_error_message() );
			return $response;
		}
		...
		if ( 200 !== $response_code ) {
			...
			$error_message = isset( $error_data['error']['message'] ) ? $error_data['error']['message'] : __( 'API request failed.', 'smart-assistant' );
			error_log( 'Smart Assistant API Error: ' . $error_message );
		...
		if ( ! isset( $data['choices'][0]['message']['content'] ) ) {
			return new \WP_Error( 'invalid_response', __( 'Invalid response from API.', 'smart-assistant' ) );
		}
